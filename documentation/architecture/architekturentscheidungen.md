...
- **Folgen:** Größeres **Frontend-Bundle** und **Laufzeitlast** auf dem Endgerät (inkl. schwächerer Hardware); Erkennungsqualität ist abhängig von Dokument und Bibliothek — **fachliche und rechtliche Korrektheit** bleiben durch Nutzerprüfung und serverseitige Regeln abgesichert, nicht durch OCR allein. Die **strukturierte Überführung** von Freitext in Antragsfelder erfolgt **nicht** durch zusätzliche Cloud-OCR auf dem Server, sondern über die separate Edge Function **`extract-info`** mit **LLM** (ADR-007). Ein späterer Wechsel zu **Cloud-OCR** statt Browser-OCR würde ein **neues oder ergänzendes ADR** sowie Anpassungen in Verteilungssicht und Datenschutz-Folgenabschätzung erfordern.

### ADR-007: LLM-Extraktion über OpenRouter (MVP); Perspektive lokales LLM

- **Status:** akzeptiert (MVP-Stand)
- **Datum:** 2026-04-26
- **Kontext:** Aus dem per OCR gewonnenen Freitext sollen **strukturierte Antragsdaten** vorgeschlagen werden. Ein reines Regelwerk ist wartungsintensiv; ein **LLM** kann Felder zuverlässiger aus unstrukturiertem Text ableiten, benötigt aber eine **API-Anbindung** und wirft **DSGVO**-Fragen (Auftragsverarbeitung, Unterauftragsverkette, ggf. Drittlandübermittlung), wenn ein externer Anbieter eingebunden wird.
- **Entscheidung:** Die Extraktion wird in der Edge Function **`extract-info`** gekapselt. Der Client sendet nur **Text** (kein API-Key im Browser). Im MVP wird **OpenRouter** (`https://openrouter.ai/api/v1/chat/completions`) als **LLM-Gateway** genutzt; das konkrete Modell richtet sich nach Projekt-Konfiguration (siehe [README.md](../../README.md)). **Geplant** ist der **Wechsel oder die Ergänzung durch ein lokales LLM**, um Datenschutz- und Souveränitätsziele besser zu treffen, ohne den fachlichen Flow (Text → Vorschläge → Nutzerprüfung → Antrag) aufzubrechen.
- **Folgen:** Zusätzlicher **externer Abhängigkeitspfad** (Verfügbarkeit, Kosten, Tokens); **Compliance-Aufwand** für produktive Nutzung; klare **Trennung** von `process-antrag` (persistenter Antrag) und reiner Hilfs-Extraktion. Bei Anbieter- oder Modellwechsel sind Verteilungssicht, Verzeichnis von Verarbeitungstätigkeiten und ggf. AV-Verträge anzupassen.
