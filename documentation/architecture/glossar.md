...
| **Pflegegrad-Nachweis** | Nachweis über den anerkannten Pflegegrad (z. B. Bescheid, Gutachten); im UI-Text auch „Pflegegutachten“; wird im MVP hochgeladen und sicher verwaltet; kann im Browser per **OCR** vorverarbeitet und optional per **LLM-Extraktion** in strukturierte Felder überführt werden ([README](README.md#zielbild-mvp), [Verteilungssicht](verteilungssicht.md), [Bausteinsicht](bausteinsicht.md), ADR-007). |
...
| **Edge Function** | Serverseitige Funktion in der **Supabase**-Laufzeit; im Projekt u. a. **`process-antrag`** für Orchestrierung (Validierung, Speicher, E-Mail) und **`extract-info`** für **LLM-Extraktion** aus Freitext ([Verteilungssicht](verteilungssicht.md), [Lösungsstrategie](loesungsstrategie.md)). |
...
| **external-Frontend** / **internal-Frontend** | Zwei Web-Oberflächen (React-SPA): Einstieg für **Nutzer:innen** bzw. für **interne Rollen**; das **external-Frontend** kann **OCR** clientseitig nutzen und optional die Edge Function **`extract-info`** für **LLM-Extraktion** aufrufen ([Bausteinsicht](bausteinsicht.md), [Architekturentscheidungen](architekturentscheidungen.md)). |
...
| **Netlify** | Hosting-Plattform für den **Vite-Produktionsbuild** des Web-Frontends: statische Auslieferung, **globales CDN**, TLS und übliche Deploy-Pipelines (Git oder CLI); ergänzt das Backend bei **Supabase** ([Verteilungssicht](verteilungssicht.md)). |
| **extract-info** | Edge Function: nimmt JSON mit Feld `text` entgegen, ruft das konfigurierte **LLM** (im MVP über **OpenRouter**) auf und liefert strukturiertes JSON für Antragsfelder; siehe [README.md](../../README.md), [Verteilungssicht](verteilungssicht.md), ADR-007. |
| **LLM-Extraktion** | Automatisierte Ableitung strukturierter Daten aus Freitext mittels **Large Language Model**; im Projekt serverseitig in **`extract-info`**; Ergebnisse sind **Vorschläge** bis zur Nutzerprüfung ([Laufzeitsicht](laufzeitsicht.md)). |
| **OCR** (*optical character recognition*, deutsch sinngemäß Texterkennung) | Im **external-frontend** eingesetzte Bibliothek zur Erkennung von Text in Bildern oder PDF-Seiten; dient z. B. als Eingabe für **LLM-Extraktion** oder manuelle Übernahme bei **Pflegegrad-Nachweisen**, typischerweise vollständig im Browser vor dem gezielten Upload ([Bausteinsicht](bausteinsicht.md), ADR-006 in [Architekturentscheidungen](architekturentscheidungen.md)). |
| **OpenRouter** | Externer **API-Gateway-Dienst** für Zugriff auf verschiedene Sprachmodelle; im MVP Anbindung aus **`extract-info`** per HTTPS ([Verteilungssicht](verteilungssicht.md), ADR-007). |
